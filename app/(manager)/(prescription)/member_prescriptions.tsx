...
export default function MemberPrescriptionsScreen() {
  ...
  const { data: prescriptions, isLoading: isLoadingPrescriptions } =
    useGetMemberPrescriptions(memberId);
  const { mutate: deletePrescription, isPending: isDeletingPrescription } =
    useDeletePrescription();
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F9F6FC]" edges={["top"]}>
      ...
      <View className="flex-row items-center justify-between px-5 pt-3 pb-4">
        ...
        <Text className="text-xl text-black font-space-bold">Đơn thuốc</Text>
        {/* Spacer */}
          ...
          <View className="gap-y-5 mb-6">
            {prescriptions.map((pres) => (
              <View
                ...
                className="bg-white border-2 border-black rounded-[24px] p-5 shadow-sm"
              >
                <View className="flex-row justify-between items-start mb-3">
                  <View className="flex-1 mr-2">
                    <Text
                      className="text-[17px] font-space-bold text-black"
                      numberOfLines={1}
                    >
                    ...
                    <View className="flex-row items-center mt-1">
                      <View
                        className={`px-2 py-0.5 rounded-md border border-black/20 mr-2 ${pres.status === "Active" ? "bg-[#A3E6A1]" : pres.status === "Completed" ? "bg-gray-200" : "bg-gray-100"}`}
                      >
                  ...
                  <View className="flex-row gap-x-2">
                    ...
                    <Pressable
                      onPress={() =>
                        router.push({
                          pathname:
                            "/(manager)/(prescription)/edit_prescription",
                          params: {
                ...
                <View className="bg-gray-50 rounded-xl p-3 border border-gray-200">
                  <Text className="text-sm font-space-bold text-black mb-1.5">
                    Gồm {pres.medicines?.length || 0} loại thuốc:
                  </Text>
                  {pres.medicines?.slice(0, 3).map((med, idx) => (
                    <Text
                      key={idx}
                      className="text-xs font-space-medium text-gray-600 mb-0.5"
                      numberOfLines={1}
                    >
