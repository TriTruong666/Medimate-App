...
// --- MOCK DATA ---
const MOCK_REMINDERS: ReminderResponse[] = [
  {
    reminderId: "r1",
    scheduleId: "1",
    memberId: "m1",
    memberName: "Nguyễn Văn A",
    scheduleName: "Uống thuốc sáng",
    reminderTime: "08:00",
    status: "Taken",
    medicines: [
      {
  ...
  },
  {
    ...
    memberId: "m1",
    memberName: "Nguyễn Văn A",
    scheduleName: "Uống thuốc tối",
    reminderTime: "22:00", // Giả sử để test nút nhắc nhở nếu gần giờ này
    status: "Pending",
    medicines: [
      {
        detailId: "d1",
...
function hasMockData(date: dayjs.Dayjs): boolean {
  if (date.isSame(dayjs(), "day")) return true;
...
export default function MemberScheduleScreen() {
  ...
  const dailyReminders = useMemo(() => {
    if (!hasMockData(selectedDate)) return [];
            ...
            <View style={{ backgroundColor: "#fff", borderWidth: 2, borderColor: BORDER_COLOR, borderRadius: 24, padding: 14, ...SHADOW_MD }}>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                {monthDays.map((day, idx) => {
                  ...
                  const isSelected = day.format("YYYY-MM-DD") === selectedDateStr;
                  const isToday = day.isSame(dayjs(), "day");
                  const hasData = isCurrentMonth && hasMockData(day);
        ...
        <View style={{ paddingHorizontal: 20 }}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
            ...
            <View style={{ backgroundColor: "#fff", borderWidth: 2, borderColor: BORDER_COLOR, borderRadius: 14, paddingHorizontal: 12, paddingVertical: 6 }}>
              <Text style={{ fontSize: 11, fontFamily: "SpaceGrotesk_700Bold" }}>{dailyReminders.length === 0 ? "TRỐNG" : `${dailyReminders.length} CỬ UỐNG`}</Text>
            </View>
          </View>
    ...
    <View style={{ flexDirection: "row" }}>
      ...
      <View style={{ flex: 1, backgroundColor: "#fff", borderWidth: 2, borderColor: "#000", borderRadius: 24, padding: 18, marginBottom: 4, ...{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 } }}>
        ...
        <View style={{ marginTop: 14, gap: 8 }}>
          ...
          {!isTaken && (
            <Pressable style={{ backgroundColor: "#000", paddingVertical: 12, borderRadius: 14, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#000" }}>
